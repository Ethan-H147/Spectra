...
static const HelpGuide HELP_GUIDES[] = {
    ...
    },
    {
        ...
        {
            {"Import a file", "Audio Analysis supplies the mono source used by this workspace."},
            {"Choose a method", "Fixed whole-file ranks one zero-padded FFT grid; time-varying STFT chooses a new set per frame."},
            {"Set, compare, export", "Choose a padded-bin or spectral-energy budget, A/B the result, then save it as WAV."},
        },
        {
            {"FFT-bin budget", "Counts and percentages refer to the one-sided, zero-padded FFT grid. Zero-padding makes the grid denser but does not add source detail."},
            ...
            {"Original / Top-N", "Compare either source with shared pause, time, progress, and stop controls."},
            {"STFT spectrogram", "Time runs left to right and frequency rises upward; it remains visible in either mode."},
        },
        "Use fixed mode for the dramatic sine-wave build-up effect; use time-varying mode when you want recognizable audio with far fewer components.",
    },
    {
        APP_PAGE_SETTINGS,
        "Settings",
        ...
        {
            {"General", "Shows startup behavior and the planned session features."},
            ...
            {"Keyboard", "Use 1-5 for workspaces, Space for synth playback, F1 for help, F11 for full screen."},
        },
